:: OwenAnimate: Offline Launcher
:: Original Author: benson#0411
:: New Author: OwenAnimate#3135
:: License: MIT
set WRAPPER_VER=1.0.0
title OwenAnimate: Offline v%WRAPPER_VER% [Initializing...]
...
echo OwenAnimate: Offline
echo A project from VisualPlugin, adapted by Benson, recreated and coded by OwenAnimate
...
:: Prevents CTRL+C cancelling (please close with 0)
if %VERBOSEWRAPPER%==no (
	if "%~1" equ "point_insertion" goto point_insertion
	start "" /B cmd /C "%~F0" point_insertion
	exit
)
...
title OwenAnimate: Offline v%WRAPPER_VER% [Checking dependencies...]
...
title OwenAnimate: Offline v%WRAPPER_VER% [Installing dependencies...]
...
:: Skipped in Safe Mode, just in case anyone is running OwenAnimate: Offline in safe mode... for some reason
...
if %ADMINREQUIRED%==yes (
	...
	if /i not "%SAFE_MODE%"=="yes" (
		...
		if /i not !ERRORLEVEL!==0 (
			...
			echo  WARNING!
			echo:
			echo  OwenAnimate: Offline needs to install its dependencies.
			echo  To do this, it must be started with Admin rights.
			echo:
			echo  Close this window and re-open Wrapper as an Admin.
			echo  ^(right-click start_owenoffline.bat and click "Run as Administrator"^)
			echo:
			pause
			exit
		)
...
if %FLASH_DETECTED%==no (
	...
	if %INCLUDEDCHROMIUM%==yes (
		...
		echo OwenAnimate: Offline will not be able to install Flash.
		...
		echo:
		if not '%flashchoice%'=='' set flashchoice=%flashchoice:~0,1%
		...
		if '%flashchoice%'=='6' goto firefox_chosen
	...
	echo To install Flash Player, OwenAnimate: Offline must kill any currently running web browsers.
	echo Please make sure any work in your browser is saved before proceeding.
	echo OwenAnimate: Offline will not continue installation until you press a key.
	echo:
	pause
	echo:
...
if %NODEJS_DETECTED%==no (
	...
	if %CPU_ARCHITECTURE%==what (
		...
		echo OwenAnimate: Offline can't tell if you're on a 32-bit or 64-bit system.
...
if %HTTPSERVER_DETECTED%==no (
	if %NODEJS_DETECTED%==yes (
		...
		if %ERRORLEVEL% == 0 (
			...
			echo:
			call npm install utilities\http-server-master -g
...
if %HTTPSCERT_DETECTED%==no (
	...
	echo watch owenanimate on youtube >> isinstalled.txt
)
popd
:after_cert_install
...
if %ADMINREQUIRED%==yes (
	...
	echo:
	echo OwenAnimateL Offline no longer needs Admin rights!
	echo Please restart normally by double-clicking.
	echo:
	echo If you saw this from running normally,
	echo OwenAnimate: Offline should continue normally after a restart.
	echo:
	pause
	exit
)
color 0f
echo All dependencies now installed^^! Continuing with OwenAnimate: Offline boot.
echo:
...
title OwenAnimate: Offline v%WRAPPER_VER% [Loading...]
...
:: Open OwenAnimate: Offline in preferred browser
if %INCLUDEDCHROMIUM%==no (
	if %CUSTOMBROWSER%=="<BROWSER_PATH>" (
		echo Opening OwenAnimate: Offline in your default browser...
		start http://localhost
	) else (
		echo Opening OwenAnimate: Offline in your set browser...
		echo If this does not work, you may have set the path wrong.
		start %CUSTOMBROWSER% http://localhost
	)
) else (
	echo Opening OwenAnimate: Offline using included Chromium...
	pushd utilities\ungoogled-chromium
	if %APPCHROMIUM%==yes (
		start chrome.exe --user-data-dir=the_profile --app=http://localhost
	) else (
		start chrome.exe --user-data-dir=the_profile http://localhost
	)
	popd
)

echo OwenAnimate: Offline has been started^^! The video list should now be open.

::::::::::::::::
:: Post-Start ::
::::::::::::::::

title OwenAnimate: Offline v%WRAPPER_VER%
:wrapperstartedcls
if %VERBOSEWRAPPER%==no ( cls )
:wrapperstarted

echo:
echo OwenAnimate: Offline v%WRAPPER_VER% running
echo A project from VisualPlugin, adapted by Benson, recreated and coded by OwenAnimate
echo:
echo DON'T CLOSE THIS CMD WINDOW^^! Use this quit option -- (0) when you're done for comedy world animations.
echo:
echo Enter 1 to reopen the video list
echo Enter 2 to open the server page
echo Enter 3 to open OwenAnimate: Offline's files
echo Enter 4 to open the settings file
echo Enter 5 to open the README
echo Enter 6 to clean up the screen
echo Enter 0 to quit OwenAnimate: Offline
:wrapperidle
set /p CHOICE=Choice:
echo:
if NOT DEFINED choice echo Time to choose.&& goto wrapperidle
if '%choice%'==' ' echo Time to choose.&& goto wrapperidle
...
if '%choice%'=='watchowenanimateonyoutubeorelseyourgrounded' goto watchlowenanimateonyoutube
...
if %CUSTOMBROWSER%=="<BROWSER_PATH>" (
	...
	if %CUSTOMBROWSER%=="" (
		...
		start %CUSTOMBROWSER% https://localhost:4343 >> NUL
	)
)
goto wrapperidle
...
:open_files
pushd ..
echo Opening the OwenAnimate: Offline folder...
start explorer.exe owenanimate-offline
popd
goto wrapperidle

:open_settings
echo Opening the settings file...
echo NOTE: Changes won't take effect until a restart of OwenAnimate: Offline.
...
:exitwrapperconfirm
echo Are you sure you want to quit OwenAnimate: Offline?
echo Be sure to save all your work.
echo Type Y to quit, and N to go back.
:exitwrapperretry
set /p EXITCHOICE= Response:
echo:
if not '%exitchoice%'=='' set exitchoice=%exitchoice:~0,1%
if '%exitchoice%'=='Yes' goto point_extraction
if '%exitchoice%'=='yes' goto point_extraction
if '%exitchoice%'=='Y' goto point_extraction
if '%exitchoice%'=='y' goto point_extraction
if '%exitchoice%'=='No' goto wrapperstartedcls
if '%exitchoice%'=='no' goto wrapperstartedcls
if '%exitchoice%'=='N' goto wrapperstartedcls
if '%exitchoice%'=='n' goto wrapperstartedcls
echo You must answer Yes or No. && goto :exitwrapperretry

:point_extraction

title OwenAnimate: Offline v%WRAPPER_VER% [Shutting down...]
...
echo OwenAnimate: Offline has been shut down.
echo This window will now close.
echo Open start_owenoffline.bat again to start OwenAnimate: Offline again.
ENDLOCAL
pause
exit